...
class CorrelationMatrixOperations(BaseCorrelationAnalysis):
    ...
    def plot_correlation_matrix_nan(self, correlation_matrix, kirchhoff_matrix, secondary_structure, positive_only,epsilon):
        ...
        # Salva la figura
        if not os.path.exists(f'images/{self.name}/2_temperature_cutoff/Matrici_Correlazione/'):
            os.makedirs(f'images/{self.name}/2_temperature_cutoff/Matrici_Correlazione/')

        plt.savefig(f'images/{self.name}/2_temperature_cutoff/Matrici_Correlazione/Correlation_MatrixNan_{positive_only}_{epsilon}.png')
        # plt.show()
